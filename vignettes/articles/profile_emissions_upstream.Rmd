...
If `chunks` is `NULL`, tiltWorkflows automatically splits your `*companies`
dataset in just  enough chunks to distribute companies evenly across the
available cores in your computer. Consider adjusting the `chunks` parameter
manually. Aim to balance memory-usage and speed. A small number of `chunks`
makes each chunk bigger and may saturate the memory. A large number of `chunks`
may take longer because of the overhead of caching each chunk.
