...
## Additional setup for Databricks
...
storage_root <- "abfss://raw@storagetiltdevelop.dfs.core.windows.net/"
```


```{r}
# ep_companies
europages_campanies_path <- paste0(storage_root, "ep_companies/")
europages_campanies_spark <- spark_read_parquet(sc, path = europages_campanies_path) %>% sparklyr::filter(to_date == '2099-12-31') |> sparklyr::select(-c("from_date","to_date","tiltRecordID"))
europages_campanies <- europages_campanies_spark |>
